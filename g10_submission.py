...
######################################################################################
class YourTeamAgent(ReflexAgent):
    def __init__(self):
        super().__init__()
        self.previousPositions = set()  # Track visited positions to prevent loops.
        self.lastDirection = None       # Store last chosen direction to prioritize consistent movement.
    ...
    def getAction(self, gameState, agentIndex=0):
        ...
        # Prevent looping by avoiding previous positions aggressively
        newLegalMoves = [move for move in legalMoves if gameState.generateSuccessor(agentIndex, move).getPacmanPosition(agentIndex) not in self.previousPositions]
        if not newLegalMoves:
            newLegalMoves = legalMoves  # If no new moves avoid old positions, allow previous moves

        scores = [self.evaluationFunction(gameState, action, agentIndex) for action in newLegalMoves]
        bestScore = max(scores)
        bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]

        chosenIndex = random.choice(bestIndices)  # Random choice among the best moves
        chosenAction = newLegalMoves[chosenIndex]

        # Update position tracking
        pacmanPosition = gameState.generateSuccessor(agentIndex, chosenAction).getPacmanPosition(agentIndex)
        if len(self.previousPositions) > 20:
            self.previousPositions.pop()
        self.previousPositions.add(pacmanPosition)
        self.lastDirection = chosenAction
    ...
    def evaluationFunction(self, currentGameState, action, agentIndex=0):
        ...
        # Prioritize closer food
        foodDistances = [manhattanDistance(newPos, food) for food in newFood.asList()]
        minFoodDistance = min(foodDistances) if foodDistances else float('inf')

        # Prioritize capsules
        capsuleDistances = [manhattanDistance(newPos, capsule) for capsule in newCapsules]
        minCapsuleDistance = min(capsuleDistances) if capsuleDistances else float('inf')

        # Check if Pacman has eaten a capsule
        scaredTimes = currentGameState.getScaredTimes()
        isPacmanPowered = scaredTimes > 0  # Correctly handle scared times for Pacman

        # Calculate opponent's scared time
        opponentIndex = (agentIndex + 1) % 2
        opponentScaredTime = currentGameState.getScaredTimes(opponentIndex)

        if isPacmanPowered:
            # Chase the opponent
            opponentPos = currentGameState.getPacmanPosition(opponentIndex)
            opponentDistance = manhattanDistance(newPos, opponentPos)
            score = successorGameState.getScore() - 2 * opponentDistance
        else:
            if opponentScaredTime > 0:
                # Escape from the opponent
                opponentPos = currentGameState.getPacmanPosition(opponentIndex)
                opponentDistance = manhattanDistance(newPos, opponentPos)
                score = successorGameState.getScore() + 2 * opponentDistance
            elif minCapsuleDistance < 3:  # If capsule is very close, prioritize it
                score = successorGameState.getScore() - 3 * minCapsuleDistance
            else:
                score = successorGameState.getScore() - 1.5 * minFoodDistance
