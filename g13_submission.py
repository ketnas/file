...
class YourTeamAgent(MultiAgentSearchAgent):
  ...
  """
  def getAction(self, gameState: GameState, agentIndex=0) -> str:
    _, bestAction = self.minimax(gameState, self.index, self.depth, float('-inf'), float('inf'))
    return bestAction
  ...
  def minimax(self, gameState: GameState, agentIndex, depth, alpha, beta):
    if gameState.isWin() or gameState.isLose() or depth == 0:
        return self.evaluationFunction(gameState, agentIndex), None

    legalMoves = gameState.getLegalActions(agentIndex)
    if agentIndex == self.index:  # Maximizer (Pacman)
        bestValue = float('-inf')
        bestAction = None
        for action in legalMoves:
            successorGameState = gameState.generateSuccessor(agentIndex, action)
            value, _ = self.minimax(successorGameState, (agentIndex + 1) % gameState.getNumAgents(), depth - 1, alpha, beta)
            if value > bestValue:
                bestValue = value
                bestAction = action
  ...
  def evaluationFunction(self, currentGameState: GameState, agentIndex):
    numFood = currentGameState.getNumFood()
    pacmanPos = currentGameState.getPacmanPosition(agentIndex)
    foodList = currentGameState.getFood().asList()

    closestFoodDistance = min([manhattanDistance(pacmanPos, foodPos) for foodPos in foodList]) if foodList else float('inf')

    otherPacmanPositions = [currentGameState.getPacmanPosition(i) for i in range(currentGameState.getNumAgents()) if i != agentIndex]
    closestOtherPacmanDistance = min([manhattanDistance(pacmanPos, otherPos) for otherPos in otherPacmanPositions]) if otherPacmanPositions else float('inf')

    avoidOtherPacmanValue = -200 if closestOtherPacmanDistance < 2 else 0

    # Check if the other Pacman is visible (not scared)
    visibleEnemies = [currentGameState.getPacmanState(i) for i in range(currentGameState.getNumAgents()) if i != agentIndex]
    chaseVisibleEnemyValue = 0
    for enemy in visibleEnemies:
      if enemy.isPacman:
        distance = manhattanDistance(pacmanPos, enemy.configuration.pos)
        if distance > 0:
          chaseVisibleEnemyValue += 200 / distance
        else:
          chaseVisibleEnemyValue += 200  # หรือค่าคงที่อื่นๆ ตามที่คุณต้องการเมื่ออยู่ในตำแหน่งเดียวกัน

    foodValue = -1.5 * numFood
    foodDistanceValue = -2 * closestFoodDistance

    return foodValue + foodDistanceValue + avoidOtherPacmanValue + chaseVisibleEnemyValue