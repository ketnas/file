...
class YourTeamAgent(MultiAgentSearchAgent):
    ...
    def __init__(self, evalFn='scoreEvaluationFunction', depth='1', agentIndex=0):
        super().__init__(evalFn, depth, agentIndex)

    def getAction(self, gameState: GameState, agentIndex=0) -> str:
        """
        Picks the best action based on the evaluation function
        """

        # Collect legal moves for Pac-Man (agentIndex 0)
        legalMoves = gameState.getLegalActions(agentIndex)
        if not legalMoves:
            return Directions.STOP

        # Calculate scores for each action using the evaluation function
        scores = [self.evaluationFunction(gameState, action, agentIndex) for action in legalMoves]

        # Find the highest score and the corresponding action
        bestScore = max(scores)
        bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
        chosenIndex = random.choice(bestIndices)  # Pick randomly among the best
    ...
    def evaluationFunction(self, currentGameState: GameState, action: str, agentIndex=0) -> float:
        """
        Evaluates potential moves based on proximity to food and ghosts
        """
        # Generate the next state from the action
        successorGameState = currentGameState.generateSuccessor(agentIndex, action)
        pacmanPos = successorGameState.getPacmanPosition(agentIndex)
        ...
        # Scared ghost bonus
        scaredBonuses = [200 / dist if dist > 0 and ghost.scaredTimer > 0 else 0
                         for dist, ghost in zip(ghostDistances, ghostStates)]

        # Combine different scoring components
        foodScore = 1 / (nearestFoodDistance + 1) * 10
        ghostPenalty = -200 / nearestGhostDistance if nearestGhostDistance != 0 and nearestGhostDistance < 3 else 0
        scaredBonus = sum(scaredBonuses)

        return successorGameState.getScore(agentIndex) + foodScore + ghostPenalty + scaredBonus
